                ...
                int select_slice, size_t select_img) // FIXME this should be moved to image_generic.*
{
    ...
    int mode = (scale <= 1) ? xmippTransformation::NEAREST : xmippTransformation::LINEAR; // If scale factor is higher than 1, LINEAR mode is used to avoid artifacts
                  ...
                  bool only_apply_shifts)
{
    ...
    if (data.getDim() < 3 && dataMode >= DATA)
    {
        ...
        if (!A.isIdentity())
        {
            ...
            applyGeometry(SplineDegree, MULTIDIM_ARRAY(*this), tmp, A, xmippTransformation::IS_NOT_INV,
                          wrap);
        }
    }
}
...
void Image<T>::applyGeo(const MDRow &row, bool only_apply_shifts, bool wrap) {
    ...
    if (data.getDim() < 3 && dataMode >= DATA)
    {
        ...
        {
            MultidimArray<T> tmp = MULTIDIM_ARRAY(*this);
            applyGeometry(xmippTransformation::BSPLINE3, MULTIDIM_ARRAY(*this), tmp, A, xmippTransformation::IS_NOT_INV,
                          wrap);
        }
    }
}
